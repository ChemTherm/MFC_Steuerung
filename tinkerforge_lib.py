...
class TFH:
    ...
    def cb_enumerate(self, uid, connected_uid, _, hardware_version, firmware_version,
        ...
        if enumeration_type == IPConnection.ENUMERATION_TYPE_DISCONNECTED:
            print(f"Disconnect detected from device: {uid} - "f"{device_identifier_types.get(device_identifier, 'unknown device type')}")
            return
        ...
        if uid not in self.devices_present.keys():
            ...
            print(f"reconnect detected from device: {uid} - " f"{device_identifier_types.get(device_identifier, 'unknown device type')}")
...
def setup_devices(config, ipcon):
    ...
    tc_list = []
    for device_name in config['CONTROL']['Tc-R']:
    ...
    device_list = {'MFC': mfc_list, 'P': pressure_list}
    return device_list
